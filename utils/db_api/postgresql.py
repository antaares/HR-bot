...
class Database:
    ...
    async def create_scheme(self):
        sql = """CREATE SCHEMA HRBOT;"""
        await self.execute(sql, execute=True)
            

    async def create_table_users(self):
        sql = """
        CREATE TABLE IF NOT EXISTS HRBOT.Resume (
    ...
    async def select_all_users(self):
        sql = "SELECT * FROM HRBOT.Resume"
        return await self.execute(sql, fetch=True)

    async def select_all_users2(self,provency):
        sql = "SELECT * FROM HRBOT.Resume WHERE provency=$1"
    ...
    async def select_7den(self,dayy,now):
        sql = "SELECT * FROM HRBOT.Resume WHERE day>=$1 AND day<=$2"
        return await self.execute(sql, dayy,now,fetch=True)


    async def select_user(self, **kwargs):
        sql = "SELECT * FROM HRBOT.Resume WHERE "
        sql, parameters = self.format_args(sql, parameters=kwargs)
        return await self.execute(sql, *parameters, fetchrow=True)


    async def count_users(self):
        sql = "SELECT COUNT(*) FROM HRBOT.Resume"
